...
c  output if iteration was successful (reduced chisq)
	    if (chisq.lt.oldchisq.OR.alamda.ge.alamdamax)
     &	      write(6,*) 'successful iteration',niter,
     &	      chisq,oldchisq,INT(LOG10(alamda))
c  check covariance matrix to make sure no lines are irrelevant to fit
	    write(6,*) 'chisq= ',niter,chisq
	    do 77 j=1,npv
	      nzero = 0
	      do 78 k=1,npv
...
c	do j=1,npv
c	write(6,'(15g10.1)') (covar(j,k),k=1,npv)
c	end do
...
c  Output final model spectrum
 ...
 98	continue
	...
	do j=1,nlines
	    NHI(j) = MIN(errmax,NHI(j))
	    bpar(j) = 0.01*(INT(bpar(j)*100.+0.5)/1)
	    if( ewline(j).lt.10000.45) CALL model(j,j,1,ndata,2) 
	end do
        call fappend(ion_name,'mod',sig_file)
        open(unit=2,file=sig_file,status='unknown')
        do 88 j=1,ndata
          write(2,'(2f12.4,3f12.6)') waveln(j),vel(j),workflux(j),
     &       noise(j),resid(j)
 88     continue
        close(2)
  ...
  97	continue
	...
	write(2,'(1x,i5,4f14.6)') nelems,redz,vhelio,
     &	  (wavehi-waveln(1))/lambda0,tau
	write(2,'(1x,a10,i5)') ion_name,nlines
        do 90 j=1,nlines
	  if( equivwidth ) 
     &    write(2,'(i3,1x,f11.2,f10.1,f9.2,f9.2,f9.2,f9.2,f8.3,f9.3)')
     &      j,MIN(errmax,NHI(j)),vline(j),bpar(j),MIN(errmax,dNHI(j)),
     &      MIN(errmax,dvline(j)),MIN(errmax,dbpar(j))
     &	    ,ewline(j),lambda0*(1.+redz)*(1.+vline(j)/2.99792458e5)
	  if( .NOT.equivwidth ) 
     &    write(2,'(i3,1x,f12.3,f11.3,f10.3,f10.3,f10.3,f10.3)')
     &      j,MIN(errmax,NHI(j)),vline(j),bpar(j),MIN(errmax,dNHI(j)),
     &      MIN(errmax,dvline(j)),MIN(errmax,dbpar(j))
 90     continue
        close(2)
