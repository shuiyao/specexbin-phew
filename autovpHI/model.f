...
c          if (iflag.eq.1) b2 = lambda0*(1.d0+redz)*     ! wavelength at center
c     &      (1.d0+vel(centpix(i))/ckms)
c          if (iflag.eq.2) b2 = lambda0*(1.d0+redz)*
c     &      (1.d0+vline(i)/ckms)
c          if (iflag.eq.1) b2 = lambda0*((1.d0+vel(centpix(i))/ckms)/
c     &	    (1.d0-vel(centpix(i))/ckms))**0.5	! wavelength at center
c          if (iflag.eq.2) b2 = lambda0*((1.d0+vline(i)/ckms)/
c     &	    (1.d0-vline(i)/ckms))**0.5
          if (iflag.eq.1) b2 = lambda0*exp(vel(centpix(i))/ckms)
          if (iflag.eq.2) b2 = lambda0*exp(vline(i)/ckms)
...
c	if (imax.eq.ndata) write(6,*) b1,b2,b3
	  ...
	  b4 = lambda0/(b2*b3)
	    ...
	    if (fitflag.eq.1) then
	      ...
	      if( x.lt.5.d0.AND.x.gt.-5.d0 ) then
		tau = tauconst*exp(-x*x)   ! optical depth
	      else 
		tau = 0.;
	      end if
	    end if
...
c	lambda = lambda0*((1.d0+veloc/ckms)/(1.d0-veloc/ckms))**0.5
        lambda = lambda0*exp(veloc/ckms)
        do 10 i=1,np,3
          b1 = parm(i+1)
c         b2 = lambda0*(1.d0+redz)*(1.d0+parm(i)/ckms)
c         b2 = lambda0*((1.d0+parm(i)/ckms)/(1.d0-parm(i)/ckms))**0.5
          b2 = lambda0*exp(parm(i)/ckms)
...
c	write(6,*) parm(i),parm(i+1),parm(i+2),lambda0,
c     &	parm(i)/ckms,exp(parm(i)/ckms),b2,b4,tau
	if(model1.gt.1.d0) write(6,*) 'PROBLEM:',
